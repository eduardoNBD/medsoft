...
@section('title', __('messages.recoveryPassword')) 
    ...
    <section class="flex flex-col flex-col-reverse md:flex-row w-full bg-gray-50 mx-auto md:my-10 md:my-auto shadow-lg overflow-hidden  h-screen w-screen ">
        <div class="w-full md:w-1/2 py-5 md:py-20 px-8 sm:px-20 h-full flex justify-between bg-white">
            <div class="my-2 md:my-auto w-full"> 
                ...
                <h2 class="text-center text-3xl mb-4 text-gray-700">{{ __('messages.recoveryPassword') }}</h2>
                <div class="px-0 md:px-28">
                    <form action="">
                        <div class="relative z-0">
                            <input type="text" autocomplete="off" id="email" name="email" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer" placeholder=" " />
                            <label for="email" class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 peer-focus:text-[#4D4E8D] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">{{ __('messages.email') }}</label>
                            <small id="email_message" class="text_error pl-2 italic"></small>
                        </div> 
                        <div class="text-center mt-5">   
                            <button class="bg-[#4D4E8D] py-1.5 px-6 text-white rounded-sm flex-grow">{{ __('messages.recoveryPassword') }}</button>
                        </div>
                    </form> 
...
@section('scripts')
    <script>
        const url = "{{$_['routes']['auth']['recovery']}}"; 
        const msg = "{{isset($_GET['msg']) ? $_GET['msg'] : ""}}";
        const redirect = "{{$_['baseURL']}}/login";
    </script>
    <script src="{{ asset('/resources/js/pages/forgetPassword.js') }}"></script> 
@stop
