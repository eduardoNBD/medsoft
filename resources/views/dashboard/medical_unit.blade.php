@extends('layouts.dashboard',[ 'current_route'  => '/dashboard/medical_units'])

@section('title', __($title)." ".$medicalUnit->name) 
...
@section('breadcrumbs')
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center">
                <a href="{{$_['baseURL'].$_['routes']['medical_units']['root']}}" class="inline-flex items-center text-sm font-medium breadcrumbs_text">
            ...
            @if($id)
                <li>
                    <span class="text-sm font-medium breadcrumbs_text">{{$medicalUnit->name}}</span>  
                </li>
            @else 
                <li>
                    <div class="flex items-center"> 
                        <span class="text-sm font-medium breadcrumbs_text">{{__('routes.medical_unit')}}</span>
                    </div>
                </li>
            @endif
        </ol>
...
@section('content') 
    @include('../components/medical_unit/form')  
@stop 

@section('scripts') 
    <script src="{{ asset('/resources/libs/multiSelectCustom/multiSelect.js') }}"></script> 
    <script>
        const url = '{{$id ? $_['baseURL']."/medical_units/update/".$id : $_['baseURL']."/medical_units/create" }}';   
        const redirect = '{{$_['baseURL'].$_['routes']['medical_units']['root']}}'; 
    </script>  
    <script src="{{ asset('/resources/js/pages/medical_unit.js') }}"></script>   
@stop