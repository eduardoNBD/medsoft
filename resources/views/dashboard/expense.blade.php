...
@section('title', __($title)." ".$expense->name) 
            ...
            @if($id)
                <li>
                    <div class="flex items-center">
                        <span class="ms-1 text-sm font-medium breadcrumbs_text">{{$expense->name}}</span>
                    </div>
                </li>
            @else 
                <li>
...
@section('scripts')  
    <script>
        const url = '{{$id ? $_['baseURL']."/expenses/update/".$id : $_['baseURL']."/expenses/create" }}'; 
        const redirect = '{{$_['baseURL'].$_['routes']['expenses']['root']}}';  
    </script>  
    <script src="{{ asset('/resources/js/pages/expense.js') }}"></script>    
@stop