<div class="bg_Secondary relative shadow-md rounded-lg">
    <div class="pt-4 px-4 table_text flex flex-col xs:flex-row justify-between gap-2">
        ...
        <div class="md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <a href="{{$_['baseURL'].$_['routes']['users']['new']}}" class=" flex items-center justify-center button_success focus:ring-4 font-medium rounded-lg text-sm px-4 py-2">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        ...
        <select name="role" id="role" onchange="getPagination(currentPage)" class="bg_secondary input_text space-y-3 md:space-y-0 md:space-x-2 p-2 w-full text-sm font-medium dark:border-gray-700 border rounded-lg">
    ...
    <table class="w-full text-sm text-left text-gray-700">
        <thead class="text-xs uppercase table_header text_table_header">
            <tr>  
                <th scope="col" class="px-2 md:px-4 py-3">{{__('messages.username')}}</th>
                <th scope="col" class="px-2 md:px-4 py-3">{{__("messages.name")}}</th>  
                <th scope="col" class="px-2 md:px-4 py-3 hidden md:table-cell">E-Mail</th> 
                <th scope="col" class="px-2 md:px-4 py-3 hidden md:table-cell">{{__("messages.phone")}}</th>  
