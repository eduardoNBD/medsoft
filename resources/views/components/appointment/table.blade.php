<div class="table_background relative shadow-md rounded-lg">
    <div class="pt-4 px-4 table_text flex flex-col xs:flex-row justify-between gap-2">
        ...
        <div class="md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <a href="{{Auth::user()->role == 3 ? $_['baseURL'].$_['routes']['appointments']['new']."/?patient=".Auth::user()->patient->id: $_['baseURL'].$_['routes']['appointments']['new']}}" class="flex items-center justify-center button_success focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        ...
        <ul class="items-center w-full text-sm font-medium dark:border-gray-700 border rounded-lg grid grid-cols-2 md:grid-cols-4">
            <li class="w-full">
                <div class="flex items-center ps-3">
                    ...
                    <label for="cancel" class="w-full py-2 ms-2 text-sm font-medium table_text">{{__("messages.canceled_her")}}</label>
                </div>
            </li>
            <li class="w-full ">
                <div class="flex items-center ps-3">
                    ...
                    <label for="pending" class="w-full py-2 ms-2 text-sm font-medium table_text">{{__("messages.pendings")}}</label>
                </div>
            </li> 
            <li class="w-full ">
                <div class="flex items-center ps-3">
                    <input id="confirmed" name="status" type="checkbox" onclick="if(checkedInputs()){ getPagination(currentPage) }else{ this.checked = true;}" value="2" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2" checked>
                    <label for="confirmed" class="w-full py-2 ms-2 text-sm font-medium table_text">{{__("messages.confirmed")}}</label>
                </div>
            </li> 
            <li class="w-full ">
                <div class="flex items-center ps-3">
                    <input id="completed" name="status" type="checkbox" onclick="if(checkedInputs()){ getPagination(currentPage) }else{ this.checked = true;}" value="3" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                    <label for="completed" class="w-full py-2 ms-2 text-sm font-medium table_text">{{__("messages.completed_her")}}</label>
                </div>
            </li> 
        </ul> 
    </div> 
    <table class="w-full text-sm text-left">
        <thead class="text-xs uppercase table_header text_table_header">
            <tr>  
                ...
                <th scope="col" class="px-4 py-3">{{__("messages.subject")}}</th>  
