<div class="bg_Secondary relative shadow-md rounded-lg">
    <div class="pt-4 px-4 table_text flex flex-col xs:flex-row justify-between gap-2">
        ...
        <div class="md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <a href="{{$_['baseURL'].$_['routes']['certificates']['new']}}" class=" flex items-center justify-center button_success focus:ring-4 font-medium rounded-lg text-sm px-4 py-2">
                <svg class="h-3.5 w-3.5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path clipRule="evenodd" fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" />
                </svg>
                {{__("messages.addCertificate")}}
            </a> 
        </div>
    </div>
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        ...
        <ul class="items-center w-full text-sm font-medium dark:border-gray-700 border rounded-lg  grid grid-cols-4">
            <li class="col-span-2 md:col-span-1">
                <div class="flex items-center ps-3">
                    ...
                    <label for="cancel" class="w-full py-2 ms-2 text-sm font-medium paragraph_text">{{__("messages.canceled_his")}}</label>
                </div>
            </li>
            <li class="col-span-2 md:col-span-1">
                <div class="flex items-center ps-3">
                    ...
                    <label for="pending" class="w-full py-2 ms-2 text-sm font-medium paragraph_text">{{__("messages.pending")}}</label>
                </div>
            </li> 
            <li class="col-span-2 md:col-span-1">
                <div class="flex items-center ps-3">
                    ...
                    <label for="confirmed" class="w-full py-2 ms-2 text-sm font-medium paragraph_text">{{__("messages.signeds_his")}}</label>
                </div>
            </li>  
            <li class="col-span-2 md:col-span-1">
                <div class="flex items-center ps-3">
                    <input id="expired" name="status" type="checkbox" onclick="if(checkedInputs()){ getPagination(currentPage) }else{ this.checked = true;}" value="3" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                    <label for="expired" class="w-full py-2 ms-2 text-sm font-medium paragraph_text">{{__("messages.expired_his")}}</label>
                </div>
            </li>  
        </ul> 
    </div> 
