<div class="bg_Secondary relative shadow-md rounded-lg">
    <div class="pt-4 px-4 table_text flex flex-col xs:flex-row justify-between gap-2">
        ...
        <div class="md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <a href="{{$_['baseURL'].$_['routes']['supplies']['new']}}" class=" flex items-center justify-center button_success focus:ring-4 font-medium rounded-lg text-sm px-4 py-2">
                <svg class="h-3.5 w-3.5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path clipRule="evenodd" fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" />
                </svg>
                {{__("messages.addSupply")}}
            </a> 
        </div>
    </div>
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        ...
        <select name="category" id="category" onchange="getPagination(currentPage)" class="bg_secondary input_text space-y-3 md:space-y-0 md:space-x-2 p-2 w-full text-sm font-medium dark:border-gray-700 border rounded-lg">
            <option value="">{{__("messages.all_his")}}</option>
            @foreach ($categories as $item)  
                <option value="{{$item->id}}">{{__("item_categories.".$item->name)}}</option>
            @endforeach
        </select> 
    </div> 
    <table class="w-full text-sm text-left">
        <thead class="text-xs uppercase table_header text_table_header">
            <tr>  
                <th scope="col" class="px-2 md:px-4 py-3 table-cell">{{__('messages.name')}}</th> 
                <th scope="col" class="px-2 md:px-4 py-3 hidden md:table-cell">{{__("routes.medical_unit")}}</th>  
                <th scope="col" class="px-2 md:px-4 py-3 table-cell">{{__("messages.category")}}</th>  
                <th scope="col" class="px-2 md:px-4 py-3 text-center hidden md:table-cell">{{__("messages.expiration")}}</th> 
                <th scope="col" class="px-2 md:px-4 py-3 hidden md:table-cell">{{__("messages.barcode")}}</th>  
