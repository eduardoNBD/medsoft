<div class="bg_secondary relative shadow-md sm:rounded-lg">
    <div class="pt-4 px-4 text-gray-700 dark:text-gray-400 flex flex-col xs:flex-row justify-between gap-2">
        ...
        <div class="md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <a href="{{$_['baseURL'].$_['routes']['services']['new']}}" class=" flex items-center justify-center button_success focus:ring-4 font-medium rounded-lg text-sm px-4 py-2">
                <svg class="h-3.5 w-3.5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                    <path clipRule="evenodd" fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" />
                </svg>
                {{__("messages.addService")}}
            </a> 
        </div>
    </div>
    ...
    <table class="w-full text-sm text-left text-gray-700">
        <thead class="text-xs uppercase table_header text_table_header">
            <tr>  
                ...
                <th scope="col" class="px-4 py-3">{{__("routes.medical_unit")}}</th>  
                <th scope="col" class="px-4 py-3">{{__("messages.category")}}</th> 
                <th scope="col" class="px-4 py-3 hidden md:table-cell">{{__("messages.price")}}</th> 
                <th scope="col" class="px-4 py-3 hidden md:table-cell">{{__("messages.barcode")}}</th>  
