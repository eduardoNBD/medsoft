<form action="#" method="POST" id="form_service"  class="grid grid-cols-1 md:grid-cols-7 gap-y-4 gap-x-0 md:gap-4 mx-4"> 
    @if($id)    
    ...
    <section class="bg_secondary px-7 pt-5 pb-5 w-full rounded-lg col-span-6 md:col-span-3 relative">
        <button type="button" id="delMain" class="button_error p-2 rounded-full absolute right-[36%] top-[3%] {{$service->image ? "" : "hidden"}}" >
            <svg  xmlns="http://www.w3.org/2000/svg"  width="18"  height="18"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-trash"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M4 7l16 0" /><path d="M10 11l0 6" /><path d="M14 11l0 6" /><path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12" /><path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3" /></svg>
        </button> 
        <div class="px-2 col-span-2 mb-4">
            <img id="img-service" width="200" height="200" alt="Image picture" class="border-2 b-[#444444] m-auto h-[150px] w-[150px] mb-4 rounded-full" src="{{ $service->image ? asset("/storage/app/").'/'.$service->image : asset('/resources/img/noimg.jpg') }}" /> 
            <div class="relative w-10/12  m-auto">
                <label title="Click to upload" for="imgService" class=" flex items-center justify-center gap-4 px-4 py-2 before:border-gray-400/60 hover:before:border-gray-300 group before:bg-gray-100 before:dark:bg-gray-700 before:absolute before:inset-0 before:rounded-3xl before:border before:border-dashed before:transition-transform before:duration-300 hover:before:scale-105 active:duration-75 active:before:scale-95">
                    ...
                    <div class="relative">
                        <span class="block text-base font-semibold relative text-gray-600 dark:text-gray-200 text-sm ">
                            {{__("messages.uploadServicePicture")}}
                        </span> 
                    </div>
                </label>
                <input autocomplete="off" class="opacity-0 absolute top-0 w-full h-full cursor-pointer" onChange="handleChangeImage()" type="file" name="imgService" id="imgService" />
            </div>
        </div> 
        <div class="col-span-4">
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 group">
                    <textarea autocomplete="off" name="description" id="description" class="block py-2 px-0 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer" placeholder="" >{{$service->description}}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 sm:mx-10 mt-4">  
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 mb-2 group">
                    <input type="text" autocomplete="off" name="name" id="name" value="{{$service->name}}" class="block py-2 px-0 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer" placeholder="" />
            ...
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 group mb-2">
                    <select name="category" id="category" class="bg_secondary block p-2 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer">
                        <option value=""></option>
                        @foreach($categories as $category)
                            <option value="{{$category->id}}" {{$service->cat_id == $category->id ? "selected" : ""}} >{{__("item_categories.".$category->name)}}</option>
                        @endforeach
                    </select> 
            ...
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 group mb-2">
                    <select name="area" id="area" class="bg_secondary block p-2 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer">
                        <option value=""></option>
                        @foreach($areas as $area)
                            <option value="{{$area->id}}" {{$service->area_id == $area->id ? "selected" : ""}} >{{__("areas.".$area->name)}}</option>
                        @endforeach
                    </select> 
                    <label for="area" class="z-10 peer-focus:font-medium absolute text-sm input_label_text duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{__("messages.area")}}</label>
                    <small id="area_message" class="text_error pl-2 italic"></small>
                </div>
            </div> 
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 group mb-2">
                    <select name="medical_unit" id="medical_unit" {{$service->medical_unit->id ? "disabled" : ""}} class="bg_secondary block p-2 w-full text-sm input_text border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer">
                        <option value=""></option>
                        @foreach($medicalUnits as $medicalUnit)
                            <option value="{{$medicalUnit->id}}" {{$service->medical_unit->id == $medicalUnit->id ? "selected" : ""}} >{{$medicalUnit->name}}</option>
                        @endforeach
                    </select> 
            ...
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 mb-2 group">
                    <input type="text" autocomplete="off" name="barcode" id="barcode" value="{{$service->barcode}}" class="block py-2 px-0 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer" placeholder="" />
            ...
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 mb-2 group">
                    <input type="text" autocomplete="off" name="price" id="price" value="{{$service->price}}" class="block py-2 px-0 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer" placeholder="" />
            ...
            <div class="px-2 col-span-2 md:col-span-1">
                <div class="relative z-0 mb-2 group">
                    <input type="text" autocomplete="off" name="commission" id="commission" value="{{$service->commission_amount}}" class="block py-2 px-0 w-full text-sm input_text bg-transparent border-0 border-b-2 input_border appearance-none focus:outline-none focus:ring-0 peer" placeholder="" />
