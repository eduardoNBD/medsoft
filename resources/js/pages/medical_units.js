    ...
    if(data.length){
        data.forEach(medicalUnit => {  
            let rowHTML = ` 
                <td class="px-2 md:px-4 py-3 font-medium table_text font-bold">
                    <div class="flex">
                        <div class="text-center flex-shrink-0 object-cover w-[30px] h-[30px] mr-3 rounded-full">
                            <img src="${medicalUnit.logo ? `${storegeBase}/${medicalUnit.logo}` : defaultUsrImg}" alt="profile" class="rounded-full w-full h-full">
                        </div>
                        <div>
                            <span class="text-xs sm:text-base">${medicalUnit.name}</span> 
                            ${medicalUnit.status == "0" ? `<small class="block text-red-600 flex-1"><i>${deletedText}</i></small>` : ""}
                        </div>
                    </div>
                </td> 
                <td class="px-2 md:px-4 py-3 font-medium table_text font-bold">
                ...
                <td class="px-2 md:px-4 py-3 table_text hidden md:table-cell text-center">${medicalUnit.fulladdress}</td> 
                        ...
                        <div class="group relative">
                            ...
                            <div class="absolute left-[-143px] top-[-7px] group-hover:block hidden z-10 w-36 table_submenu rounded divide-y divide-gray-100 shadow border-[1px] border-gray-300 dark:border-slate-800 overflow-hidden">
                                 <ul class="text-sm table_text">
                                    <!--<li>
                                        <button onclick='detailMedicalUnit(${JSON.stringify(medicalUnit)})' class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-eye"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" /></svg>
                                            ${detailText}
                                        </button>
                                    </li>-->   
                                    ${medicalUnit.status == 1 ? `<li>
                                        <a href="${medicalUnitEditURL+medicalUnit.id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                            ${editText}
                                        </a> 
                                    </li>` : ""}
                                    ${medicalUnit.id != ci && medicalUnit.status == 1 ?`<li>
                                        <button onclick="deleteMedicalUnits('${medicalUnit.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                    ...
                                    `<li>
                                        <button onclick="restoreMedicalUnits('${medicalUnit.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
...
function deleteMedicalUnits(id){
    currentItem = id;
    openModal("#deleteModal");
}

function restoreMedicalUnits(id){
    currentItem = id;
