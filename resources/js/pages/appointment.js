...
$("#datepicker").addEventListener('changeDate', (event) => {  
    if(event.detail.date != undefined){
        if(checkDate(event.detail.date)){
            ...
            fetch(urlGetAvailable+$("#doctor").value, { 
                method: "POST",
                body: formData,
                headers: { 
            ...
            .then((json) => { 
                ...
                if(json.availableSlots.length){
                    $("#timetable").innerHTML = json.availableSlots.map(slot => `
                        <li>
                            <input type="radio" id="${slot.replace(":","-")}" value="${slot}" ${appHour == slot ? "checked" : ""} class="hidden peer" name="timetable" onchange="handleRadioChange(event)">
...
$("#doctor").addEventListener('change', (event) => {  
    if(event.target.value != ""){ 
        $("#rowDateTime").classList.remove("hidden");
        ...
        if(currentDoctor.offer_discount){
            $(".discountRow").forEach(element => {
        ...
        $("#medical_unit").innerHTML = `<option value=""></option>
            ${currentDoctor.medical_units.map( medicalunit => 
                `<option value="${medicalunit.id}" ${medicalunit.id == appointment.medical_unit_id ? "selected" : ""}>${medicalunit.name}</option>`
            ).join(",")}`;

    }else{
        $("#rowDateTime").classList.add("hidden");
        $("#patient").innerHTML = "";
        $("#first_name").value = "";
        $("#last_name").value  = "";
        $("#email").value      = "";
        ...
        $("#datepicker").datepicker.picker.controls.clearBtn.click(); 
...
window.addEventListener('load', () => { 
    ...
    if(appDate){ 
        $("#datepicker").datepicker.setDate(reformatDate(appDate,"d/m/y"));
    }

    if(doctor && $("#doctor").value == "" && appointment.origin != 3){ 
        $("#doctor").value = doctor;
