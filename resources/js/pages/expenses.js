        ...
        data.forEach(expense => {  
            let rowHTML = ` 
                <td class="px-4 py-3 font-medium table_text font-bold"> 
                    ${expense.name} 
                    ${expense.status == "0" ? `<small class="block text_error flex-1"><i>${deletedText}</i></small>` : ""}
                </td>   
                <td class="px-4 py-3 font-medium table_text font-bold">  
                    ${expense.description ?`<div>${expense.description}</div>` : ""}
                </td>   
                <td class="px-4 py-3 table_text text-xs md:text-md hidden md:table-cell">${expense.barcode}</td> 
                <td class="px-4 py-3 flex items-center justify-end">
                    ${pr == 1 ?`<div class="group relative">
                        <button class="inline-flex items-center p-0.5 text-sm font-medium text-center hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                        ...
                        <div class="absolute left-[-143px] top-[-7px] group-hover:block hidden z-10 w-36 table_submenu rounded divide-y divide-gray-100 shadow border-[1px] border-gray-300 dark:border-slate-800 overflow-hidden">
                             <ul class="text-sm table_text"> 
                                ${expense.status == 1 ? `<li>
                                    <a href="${expenseEditURL+expense.id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                        <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                        ${editText}
                                    </a>
                                </li>` : ""}
                                ${expense.status == 1 ?`<li>
                                    <button onclick="deleteExpense('${expense.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                ...
                                ${expense.status == 0 ? 
                                `<li>
                                    <button onclick="restoreExpense('${expense.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
