...
function getPagination(currentPage){ 
    ...
    fetch(url+currentPage+'&s='+s+'&'+status+'&'+'category='+$("#category").value, { 
        headers: { 
            ...
            let rowHTML = ` 
                <td class="px-2 md:px-4 py-3 table_text font-bold">
                    <div class="flex items-center">
                        <div class="text-center flex-shrink-0 object-cover w-[30px] h-[30px] mr-3 rounded-full hidden md:block">
                            <img src="${supply.image ? `${storegeBase}/${supply.image}` : defaultImg}" alt="profile" class="rounded-full w-full h-full">
                        </div>
                        <div>
                            ${supply.name}  
                            ${supply.description ?`<div>${supply.description}</div>` : ""}
                            ${supply.status == "0" ? `<small class="block text-red-600 flex-1"><i>${deletedText}</i></small>` : ""}
                        </div>
                    </div>
                </td> 
                <td class="px-2 md:px-4 py-3 table_text font-bold">${supply.medical_unit} </td>
                <td class="px-2 md:px-4 py-3 table_text font-bold hidden md:table-cell">${supply.category} </td> 
                <td class="px-2 md:px-4 py-3 table_text font-bold hidden md:table-cell text-center">
                    ${supply.expiration 
                        ? localeApp == "es" ? reformatDate(supply.expiration , "d c M y") : reformatDate(supply.expiration ,"M d, y")
                        : "N/A"
                    }
                    <br>
                    <small><strong>${quantityText}:</strong> ${supply.total_remaining}</small>
                </td>
                <td class="px-2 md:px-4 py-3 table_text text-xs md:text-md hidden md:table-cell">${supply.barcode}</td> 
                        ...
                        ${pr == 1 ?`<div class="group relative">
                            ...
                            <div class="absolute left-[-143px] top-[-7px] group-hover:block hidden z-10 w-36 table_submenu rounded divide-y divide-gray-100 shadow border-[1px] dark:border-slate-800 overflow-hidden">
                                 <ul class="text-sm table_text">
                                    <li class="">
                                        <a href="${supplyDetailURL+supply.id}/1" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-eye"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" /></svg>
                                            ${detailText}
                                        </a>
                                    </li>  
                                    ${supply.status == 1 ? `<li>
                                        <a href="${supplyEditURL+supply.id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                            ${editText}
                                        </a>
                                    </li>` : ""}
                                    ${supply.status == 1 ?`<li>
                                        <button onclick="deleteSupply('${supply.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                    ...
                                    ${supply.status == 0 ? 
                                    `<li>
                                        <button onclick="restoreSupply('${supply.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
