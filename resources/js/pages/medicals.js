    ...
    if(data.length){
        data.forEach(medical => {  
            let rowHTML = `  
                <td class="px-4 py-3 font-medium table_text font-bold">
                    <div class="flex">
                        <div class="text-center flex-shrink-0 object-cover w-[30px] h-[30px] mr-3 rounded-full">
                            <img src="${medical.image ? `${storegeBase}/${medical.image}` : defaultUsrImg}" alt="profile" class="rounded-full w-full h-full">
                        </div>
                        <div>
                            <a class="text-xs sm:text-base" href="${medicalDetailURL+medical.user_id}">${medical.fullname}</a>  
                            <p class="text-[10px] block md:hidden">${medical.license}</p> 
                            ${medical.status == "0" ? `<small class="block text_error flex-1"><i>${deletedText}</i></small>` : ""}
                        </div>
                    </div>
                </td>
                <td class="px-4 py-3 font-medium table_text font-bold hidden md:table-cell">${medical.license}</td>
                <td class="px-4 py-3 font-medium table_text font-bold">
                    <div class="flex gap md:gap-2 items-center mb-2 text-xs md:text-base">
                    ...
                    <div class="flex gap md:gap-2 items-center text-xs md:text-base"> 
                ...
                <td class="px-4 py-3 table_text text-md hidden md:table-cell">
                    ${medical.start_time && medical.end_time ? `<div class="flex gap-2 items-center mb-2"> 
                        <svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="currentColor" ><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M17 3.34a10 10 0 1 1 -14.995 8.984l-.005 -.324l.005 -.324a10 10 0 0 1 14.995 -8.336zm-5 2.66a1 1 0 0 0 -.993 .883l-.007 .117v5l.009 .131a1 1 0 0 0 .197 .477l.087 .1l3 3l.094 .082a1 1 0 0 0 1.226 0l.094 -.083l.083 -.094a1 1 0 0 0 0 -1.226l-.083 -.094l-2.707 -2.708v-4.585l-.007 -.117a1 1 0 0 0 -.993 -.883z" /></svg>
                        ${medical.start_time.substring(0,5)} ${parseInt(medical.start_time.substring(0,2)) < 12 ? "am" : "pm"}
                    </div>
                    <div class="flex gap-2 items-center"> 
                        <svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="currentColor" ><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M17 3.34a10 10 0 1 1 -14.995 8.984l-.005 -.324l.005 -.324a10 10 0 0 1 14.995 -8.336zm-5 2.66a1 1 0 0 0 -.993 .883l-.007 .117v5l.009 .131a1 1 0 0 0 .197 .477l.087 .1l3 3l.094 .082a1 1 0 0 0 1.226 0l.094 -.083l.083 -.094a1 1 0 0 0 0 -1.226l-.083 -.094l-2.707 -2.708v-4.585l-.007 -.117a1 1 0 0 0 -.993 -.883z" /></svg>
                        ${medical.end_time.substring(0,5)} ${parseInt(medical.end_time.substring(0,2)) < 12 ? "am" : "pm"}
                    </div>`: noService}
                </td> 
                <td class="px-4 py-3 table_text text-md hidden md:table-cell">
                    ${medical.medical_units.map(medicalUnit => `<div>${medicalUnit.name}</div>`).join("")}
                </td> 
                <td class="px-4 py-3">
                    ${pr == '1' ?
                        ...
                        <div class="group relative">
                            ...
                            <div class="absolute left-[-143px] top-[-7px] group-hover:block hidden z-10 w-36 table_submenu rounded divide-y divide-gray-100 shadow border-[1px] border-gray-300 dark:border-slate-800 overflow-hidden">
                                 <ul class="text-sm table_text">
                                    <!--<li>
                                        <a href="${medicalDetailURL+medical.user_id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-eye"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" /></svg>
                                            ${detailText}
                                        </a>
                                    </li> -->  
                                    ${medical.status == 1 ? `<li>
                                        <a href="${medicalEditURL+medical.user_id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                            ${editText}
                                        </a>
                                    </li>` : ""}
                                    ${medical.user_id != ci && medical.status == 1 ?`<li>
                                        <button onclick="deleteUser('${medical.user_id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                    ...
                                    ${medical.status == 0 ? 
                                    `<li>
                                        <button onclick="restoreUser('${medical.user_id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
