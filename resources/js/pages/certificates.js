        ...
        data.forEach(certificate => {  
            let rowHTML = ` 
                <td class="px-4 py-3 font-medium table_text font-bold">
                    ...
                    <small class="block ${certificateStatusColors[certificate.status]} flex-1"><i>${certificateStatus[certificate.status]}</i></small>
                </td> 
                <td class="px-4 py-3 font-medium table_text font-bold">${certificate.fullnameDoctor}</td>
                <td class="px-4 py-3 font-medium table_text font-bold">${certificate.notes ?? ""}</td>
                <td class="px-4 py-3 font-medium table_text font-bold hidden md:table-cell text-center ">
                    ${localeApp == "es" ? reformatDate(certificate.created_at, "h:i, d c M Y") : reformatDate(certificate.created_at,"h:i, M d Y")}
                </td>
                <td class="px-4 py-3">
                    <div class="flex items-center justify-end">
                        <div class="group relative">
                            ...
                            <div class="absolute left-[-143px] top-[-7px] group-hover:block group-hover:block hidden z-10 w-36 table_submenu rounded divide-y divide-gray-100 shadow border-[1px] dark:border-slate-800 overflow-hidden">
                                 <ul class="text-sm table_text">
                                    <li>
                                        <a href="${urlContent}${certificate.id}" target="_blank" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                    ...
                                    ${certificate.status == 1 ? `<li>
                                        <a href="${certificateEditURL+certificate.id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                            ${editText}
                                        </a>
                                    </li>` : ""} 
                                    ${certificate.status != 0 && pr != 3 ?
                                    `<li>
                                        <button onclick="cancelCertificate('${certificate.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
