...
function getPagination(currentPage){ 
    ...
    fetch(url+currentPage+'&s='+s+'&'+status+'&'+'role='+$("#role").value, { 
        headers: { 
...
function setRows(data){ 
    ...
    if(data.length){
        data.forEach(user => {  
            let rowHTML = ` 
                <td class="px-2 md:px-4 py-3 font-medium table_text font-bold flex">
                    <div class="text-center flex-shrink-0 object-cover w-[30px] h-[30px] mr-3 rounded-full">
                        <img src="${user.image ? `${storegeBase}/${user.image}` : defaultUsrImg}" alt="profile" class="rounded-full w-full h-full">
                    </div>
                    <div>
                        <span class="text-xs sm:text-base">${user.username}</span>
                        <small class="block"><i>${roles[user.role]}</i></small>
                        ${user.status == "0" ? `<small class="block text_error flex-1"><i>${deletedText}</i></small>` : ""}
                    </div>
                </td> 
                <td class="px-2 md:px-4 py-3 font-medium table_text font-bold">
                    <span class="text-xs sm:text-base">${user.fullname}</span>
                    <a href="mailto:${user.email}" class="text-[10px] block sm:hidden">${user.email}</a> 
                </td>
                <td class="px-2 md:px-4 py-3 font-medium table_text font-bold hidden md:table-cell">${user.email}</td>
                <td class="px-2 md:px-4 py-3 table_text text-xs md:text-md hidden md:table-cell">${user.phone ?? ""}</td> 
                <td class="px-2 md:px-4 py-3 flex items-center justify-end">
                    ${(user.role == 1 && pr == '1') || user.role != 1 ?`<div class="group relative">
                        ...
                        <div class="absolute left-[-200px] top-[-7px] group-hover:block hidden z-10 w-[13rem] table_submenu rounded divide-y divide-gray-100 shadow border-[1px] border-gray-300 dark:border-slate-800 overflow-hidden">
                             <ul class="text-sm table_text">
                                <li>
                                    <button onclick='detailUser(${JSON.stringify(user)})' class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                        <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-eye"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" /></svg>
                                        ${detailText}
                                    </button>
                                </li>  
                                ${user.status == 1 ? `<li>
                                    <a href="${userEditURL+user.id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                        <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                        ${editText}
                                    </a>
                                </li>
                                <li>
                                    <button onclick="changePassword('${user.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                ...
                                ${user.id != ci && user.status == 1 ?`<li>
                                    <button onclick="deleteUser('${user.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                ...
                                ${user.status == 0 ? 
                                `<li>
                                    <button onclick="restoreUser('${user.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
...
function changePassword(id){
    ...
    $("#changePassword").querySelector(`#new_password_message`).innerHTML = "";
    $("#changePassword").querySelector(`#new_password_confirmation_message`).innerHTML = "";
    $("#changePassword").querySelector(`#new_password`)?.classList.remove("border-red-500");
    $("#changePassword").querySelector(`#new_password_confirmation`)?.classList.remove("border-red-500");
    $("#changePassword").querySelector(`#new_password+label`)?.classList.remove("text_error");
    $("#changePassword").querySelector(`#new_password_confirmation+label`)?.classList.remove("text_error");
...
function confirmChangePassword(){
    ...
    data.append("new_password_confirmation", $("#new_password_confirmation").value)
    
    fetch(urlChangePassword+currentItem, {  
        method: "post", 
        body: data,
        headers: { 
        ...
        if(json.status){  
            ...
            if(typeof json.message == "object"){
                Object.keys(json.message).forEach(key => {  
                   $("#changePassword").querySelector(`#${key}_message`).innerHTML = json.message[key];
                   $("#changePassword").querySelector(`#${key}`)?.classList.add("border-red-500");
                   $("#changePassword").querySelector(`#${key}+label`)?.classList.add("text_error");
                });
            }else{
                createAlert(json.message, "error")
            }
...
function detailUser(user){ console.log(user.status);
    let status = `<span class="inline-block h-2 w-2 rounded-full ${user.status == 1 ? 'bg-emerald-500' : 'bg-red-500'} ml-2" title="status">
    ...
    if(user.role == 2){   
        ...
        $("#doctorSpecialties").innerHTML = user.specialties.map(specialty => 
            `<span class="text-xs font-medium me-2 px-2.5 py-0.5 rounded button_primary">
                ${specialty}
            </span>`).join("");
        ...
        $("#medicalUnits").innerHTML = user.medical_units.map(medicalUnit => medicalUnit.name).join("<br>");
    }

    if(user.role == 3){
        $("#patientAddress").innerHTML = `<br>${user.address} ${user.city} ${user.zipcode} ${user.city}  ${user.country}`; 
