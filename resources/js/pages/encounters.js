        ...
        data.forEach(encounter => {  
            let rowHTML = ` 
                <td class="px-4 py-3 font-medium table_text font-bold">
                    <span class="text-xs sm:text-base">${encounter.fullnamePatient}</span>
                    <small class="block ${encounterStatusColors[encounter.status]} flex-1"><i>${encounterStatus[encounter.status]}</i></small>
                </td> 
                <td class="px-4 py-3 font-medium table_text font-bold hidden md:table-cell ">
                    <span class="text-xs sm:text-base">${encounter.fullnameDoctor}</span>
                </td>
                <td class="px-4 py-3 font-medium table_text font-bold max-w-[80px] sm:max-w-fit overflow-hidden">
                    <span class="text-xs sm:text-base">${encounter.subject}</span>
                </td>
                <td class="px-4 py-3 font-medium table_text font-bold hidden md:table-cell text-center ">${localeApp == "es" ? reformatDate(encounter.date, "h:i, d c M Y") : reformatDate(encounter.date,"h:i, M d Y")}</td>
                <td class="px-4 py-3 table_text hidden md:table-cell"> 
                    <div class="flex justify-evenly"> 
                        <strong>Subtotal:</strong> $ ${customNumberFormat(parseFloat(encounter.subtotal)+encounter.subtotalItems)}
                    </div> 
                    <div class="flex justify-evenly"> 
                        ${encounter.discount != 0 ? `<strong>${discountText}:</strong> $ ${customNumberFormat(encounter.discount)}` : ""}
                    </div>  
                </td> 
                <td class="px-4 py-3  max-w-[40px]">
                    <div class="flex items-center justify-end">
                        ${encounter.status != 0 ? `<div class="group relative">
                            ...
                            <div class="absolute left-[-143px] top-[-7px] group-hover:block hidden z-10 w-36 table_submenu rounded divide-y divide-gray-100 shadow border-[1px] dark:border-slate-800 overflow-hidden">
                                <ul class="text-sm table_text">
                                    <li>
                                        <a href='${detailUrl+encounter.id}' class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-eye"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0" /><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6" /></svg>
                                            ${detailText}
                                        </a>
                                    </li>
                                    ${encounter.status != 2 && encounter.status != 0 ? `<li>
                                        <a href="${encounterEditURL+encounter.id}" class="block w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
                                            <svg  xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-edit"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7h-1a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-1" /><path d="M20.385 6.585a2.1 2.1 0 0 0 -2.97 -2.97l-8.415 8.385v3h3l8.385 -8.415z" /><path d="M16 5l3 3" /></svg>
                                            ${editText}
                                        </a>
                                    </li>` : ""}
                                    ${encounter.status != 2 && encounter.status != 0 ?
                                    `<li>
                                        <button onclick="cancelEncounter('${encounter.id}')" class="w-full text-left submenu_text font-medium py-2 px-4 button_table_submenu flex items-center gap-2">
